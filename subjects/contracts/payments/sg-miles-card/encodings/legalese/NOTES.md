## 0. Filing note: why one leaf holds eight issuers' instruments

This encoding was produced in `legalese/l4-ide` (the L4 tooling monorepo), at
`jl4/examples/legal/miles-card/`, driven by that repo's `etc/go/go.sh` pipeline, under the
sidecar name `sg-miles-card`. It landed in that repo's `unstable` branch via PRs
[#437](https://github.com/legalese/l4-ide/pull/437) (a Catala-projection pipeline leg),
[#438](https://github.com/legalese/l4-ide/pull/438) (four Catala exporter fixes) and
[#439](https://github.com/legalese/l4-ide/pull/439) (the corpus itself, merge commit
`9c93a028a`), then deposited here on 2026-09-21.

**This row departs from `docs/directory-conventions.md` §3's stated rule** — "distinct
instruments in a family are distinct leaves" — because eight banks' reward-programme T&Cs
are encoded against one shared domain module (`miles-card-domain.l4`: one `Card` enum, one
`Transaction`, one `Rate answer` shape) and one composer (`miles-card.l4`) that dispatches
across all eight, purpose-built so a consumer app can ask "which card earns the most on
*this* purchase" across the whole set in one call. Splitting into eight leaves would either
duplicate the shared ontology eight times or strand the comparison capability entirely —
neither of which the rule's own worked example (a SAFE under two governing laws, i.e. one
instrument, not eight) actually contemplates.

The precedent this follows instead: `subjects/sg/succession` (four Acts of Parliament, one
shared family-law ontology, one leaf, its `subject.json.encodings_note` stating the same
kind of departure) and `subjects/il/ofek-hadash-2008` (several collective agreements, one
leaf). Both are comparisons-across-instruments, same as this one. Meng ruled the bundling
shape on 2026-09-21, when this deposit happened, choosing it over "leave it in l4-ide" and
"eight separate leaves with the domain module duplicated or dropped."

One further departure this filing needed: none of the four seed genres in
`contracts/GENRES.md` (`insurance`, `investment`, `leasing`, `lending`) holds a card's
*rewards programme* as distinct from the credit facility it rides on, so `payments/` was
added to that controlled list in the same change, with its own rationale recorded there.
...
The cheat sheet (`[S5]`) and the original app (`which-card.alexis-original.html` in homelab) belong to
Alexis and are read-only; Meng's fork `which-card.html` beside it takes the generated `D[]`;
neither is committed here, in either repository.
The T&C PDFs are the issuers' published documents; a mirrored copy for versioning lives in
`legalese/l4-ide` (see §8), not in this repository (§0).
The L4 is Meng's. **This file's own prediction was wrong**: it originally read "moves to
`legalese/canon` under `subjects/sg/miles-card/` once it lands here" — `sg/` is for enacted
law (docs/directory-conventions.md §2), and eight issuers' private T&C are not that. The
actual landing path, decided when this migration happened, is `contracts/payments/` (§0).


---

## 8. Where this came from, and what does not travel with it

Produced by an agent session (`miles-encoding`, transcript UUID
`2bec56ac-d6f8-4426-8d0c-430fe9cac48f`) working in `legalese/l4-ide` worktree
`~/src/legalese/l4wt/miles-card`, branch `miles-card`. **The go pipeline itself
(`etc/go/go.sh`, its phase scripts, its selftest) is NOT vendored into this repository** —
it is l4-ide tooling, not part of the encoding. The `report/go-run-*` files in this row are
its committed OUTPUT (the run that produced the HG1 payload in `gates/`), not something
this repository can re-run standalone. To regenerate goldens, re-run the acceptance
encoding, or re-run the Catala projection leg, check out `legalese/l4-ide` at or after
commit `1dd165f9c` and follow that repository's own `CLAUDE.md` §3 and this row's original
home at `etc/go/subjects/sg-miles-card/`.

Two things fixed in l4-ide but not yet re-run against this deposited copy, since they are
prose-only and touch no `.l4`: PR [#446](https://github.com/legalese/l4-ide/pull/446)
corrected two claims this file's own text used to carry (a stale "goldens are stale" note in
