...
No s 404 (dishonest misappropriation of property possessed by a deceased person),
...
s 403 and ss 407–409** and has not been re-run since; it is a ten-file bundle and the row now has
twelve modules, so it does not have the right number of FILES either and cannot answer about
s 403 or ss 407–409 at all. (For what it is worth, `grep -ch '^@export' *.l4` gives
PLACEHOLDER-EXPORTS now and gave 34 before the s 392 implication — which is not 37 at any point,
so the two numbers are not
...
re-granularisation, s 392 as an implication, s 380, s 403, or ss 407–409 — so
`tests/robbery-390-392.*.golden`, `tests/theft-378-379.*.golden` and
`tests/charge-sheet.*.golden` predate them and will not match until someone re-blesses them.
