...
`JL4_LIBRARY_PATH` pinned to `jl4-core/libraries`; the whole row was re-run module by module on
2026-09-21 with the getaway fixtures in place, under the snapshot binary named in that section
below, and the table is that run:
...
**179 in total**, re-counted on 2026-09-21 with `grep -ch '^#ASSERT' *.l4` after the getaway fixtures below; it was 157 before them.
...
**Anchor the grep.** The unanchored `grep -c '#ASSERT' *.l4` this line carried until 2026-09-21 gave the same answer for as long as no comment in the row named the directive. One comment in `robbery-390-392.l4` now does, so it returns **180** for 179 assertions. The anchored form above is the one to use.
...
the verdict the asserts state. **That deploy predates the s 392 implication and the getaway fixtures** and has not been
...
re-granularisation, s 392 as an implication, or the getaway fixtures — so `tests/robbery-390-392.*.golden` and
